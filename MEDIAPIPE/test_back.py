...
while cap.isOpened():
    ret, frame = cap.read()
    if not ret:
        print("[ERROR] Failed to capture frame from DroidCam.")
        break

    # OPTIONAL: Uncomment if rotated
    ...
    if results.multi_hand_landmarks:
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # ➤ BEAUTIFULLY draw hand on actual camera frame
            for finger_idx, joint_indices in enumerate(finger_joints):
                color = finger_colors[finger_idx]
                for j in range(len(joint_indices) - 1):
                    start = landmark_points[joint_indices[j]]
            ...
            print(f"[PREDICTION] Conf: {confidence:.2f} | Vector: {prediction}")
            ...
            # Also show on camera feed
            cv2.putText(frame, text, (10, 40), font, 0.8, (0, 255, 0), 2, cv2.LINE_AA)

    else:
        print("[INFO] No hand detected.")

    cv2.imshow("Processed for Prediction (Black BG)", black_bg)
    cv2.imshow("Actual DroidCam Feed", frame)
